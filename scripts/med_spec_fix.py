...
def save_artifacts(clf, labels, artifacts_dir="artifacts"):
    ...
    # encoder name
    open(os.path.join(artifacts_dir, "encoder_name.txt"), "w", encoding="utf-8").write(
        ENCODER + "\n"
    )
    # label list
    ...
    json.dump(
        labels_sorted,
        open(os.path.join(artifacts_dir, "label_list.json"), "w", encoding="utf-8"),
    )
    # config
    cfg = dict(
        max_len=MAX_LEN,
        ...
        classifier=dict(
            kind="LinearSVC", C=C_SVM, loss=LOSS_SVM, class_weight="balanced"
        ),
    )
    yaml.safe_dump(
        cfg, open(os.path.join(artifacts_dir, "config.yaml"), "w", encoding="utf-8")
    )
    # version
    try:
        sha = (
    ...
    open(os.path.join(artifacts_dir, "version.txt"), "w", encoding="utf-8").write(
        f"v0.1.0+{sha}  {ts}\n"
    )
...
def run_fixed_pipeline(do_tsne=True, do_confusion=True):
    ...
    df_filtered, rare_list_or_none, used = handle_rare_classes(
        df, label_col="label", k=MIN_PER_CLASS, strategy=RARE_STRATEGY
    )
    ...
    mdl = AutoModel.from_pretrained(ENCODER).to(
...
if __name__ == "__main__":
    ...
    ap.add_argument("--tsne_only", action="store_true", help="Only plot t-SNE 3D.")
    ap.add_argument("--no_tsne", action="store_true", help="No t-SNE.")
    ap.add_argument("--no_confusion", action="store_true", help="No confusion matrix.")
    args = ap.parse_args()
