...
def load_data(path="mtsamples_clean.csv"):
    ...
    if "text" in df.columns:
        ...
        if "transcription" not in df.columns:
            raise ValueError(
                "Missing both 'text' and 'transcription' columns in the CSV"
            )
...
def split_data(
    df, test_size=0.2, seed=42, label_col="label", text_col="text", n_len_bins=4
):
...
def eval_frozen_encoder(
    ...
    mdl = AutoModel.from_pretrained(encoder_name).to(
        "cuda" if torch.cuda.is_available() else "cpu"
    )

    X_train = encode_texts(
        train_df["text"].tolist(), tok, mdl, batch_size=batch_size, max_len=max_len
    )
    X_test = encode_texts(
        test_df["text"].tolist(), tok, mdl, batch_size=batch_size, max_len=max_len
    )
    ...
    if clf_type == "svm":
        ...
        param_grid = {"C": [7, 10, 15, 100], "loss": ["hinge", "squared_hinge"]}
        ...
        if penalty == "l1":
            ...
            param_grid = {"C": [0.01, 0.1, 1, 5, 10], "l1_ratio": [0.0, 0.5, 1.0]}
    ...
    title = (
        f"Confusion Matrix - Frozen({clf.__class__.__name__})\n{encoder_name}\n{save_tag}"
        if save_tag
        else f"Confusion Matrix - Frozen({clf.__class__.__name__})"
    )
...
def main():
    ...
    ap.add_argument(
        "--clf", type=str, choices=["svm", "logreg_l2", "logreg_l1"], default="svm"
    )
    ap.add_argument(
        "--min_per_class",
        type=int,
    ...
    ap.add_argument(
        "--rare_strategy",
        type=str,
        choices=["drop", "other", "train_only"],
        default="drop",
        help="How to handle classes with less than k instances: drop/other/train_only",
    )
    ap.add_argument(
    ...
    ap.add_argument(
        "--no_truncate", action="store_true", help="Do not truncate pre-cleaned texts"
    )
    ...
    df_filtered, rare_list_or_none, used = handle_rare_classes(
        df, label_col="label", k=args.min_per_class, strategy=args.rare_strategy
    )
    ...
    if used == "train_only" and rare_list_or_none:
        test_df = test_df[~test_df["label"].isin(rare_list_or_none)].reset_index(
            drop=True
        )
